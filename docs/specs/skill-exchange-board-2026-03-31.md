# Collaboration Organize Board (Guest Lectures)
...
### SF1: Lecture Board & Discovery

| ID | Requirement | Priority |
|----|------------|----------|
| SKL-BOARD-01 | Two clear tabs: "Requests from Universities" and "Offers from Nomads" | Must |
| SKL-BOARD-02 | Both tabs show cards with key info at a glance | Must |
| SKL-BOARD-03 | Combined "All Opportunities" view available as a third option | Should |

### SF2: Posting (Requests & Offers)

| ID | Requirement | Priority |
|----|------------|----------|
| SKL-POST-01 | University request fields: topic/title, detailed description, preferred date range, expected audience size, format (talk, workshop, panel, Q&A), compensation type (paid, free, or facility access in exchange), university name, department, contact person | Must |
| SKL-POST-02 | University admins can create, edit, and close their requests | Must |
| SKL-POST-03 | Status indicator on each request: Open, In Discussion, Matched, Completed, Cancelled | Must |
| SKL-POST-04 | Nomad offer fields: session title, short description, detailed topics covered, preferred format, estimated duration, compensation expectations, availability dates, relevant links (portfolio, LinkedIn, previous talks) | Must |
| SKL-POST-05 | Nomads can create, edit, and withdraw their offers | Must |
| SKL-POST-06 | Skills tags from nomad profile auto-suggested when creating an offer | Should |

### SF3: Matching & Application Flow

| ID | Requirement | Priority |
|----|------------|----------|
| SKL-MATCH-01 | Universities can "Invite" a nomad who has posted an offer — sends notification to the nomad | Must |
| SKL-MATCH-02 | Nomads can "Apply" to a university request — sends notification to the university | Must |
| SKL-MATCH-03 | When either action happens, both parties see the match in a "My Connections" section | Must |
| SKL-MATCH-04 | Simple messaging: the invitation/application includes a short message from the sender | Should |
| SKL-MATCH-05 | Status updates as the match progresses: Applied → In Discussion → Confirmed → Completed | Must |

### SF4: Feedback System

| ID | Requirement | Priority |
|----|------------|----------|
| SKL-FEEDBACK-01 | After a session is marked Completed, both sides can leave feedback | Must |
| SKL-FEEDBACK-02 | Rating: 1-5 stars + optional written comment | Must |
| SKL-FEEDBACK-03 | Nomad ratings contribute to their trust score (+3 per 4-5 star, -2 per 1-2 star rating) | Must |
| SKL-FEEDBACK-04 | Feedback is visible on the nomad's profile (aggregated average + recent comments) | Should |
...
- `LectureDetail` (Server) — Page-level for viewing a posting
...
- Universities and nomads can post and browse opportunities easily (SKL-BOARD-01, SKL-BOARD-02, SKL-POST-01, SKL-POST-04).
- Invite and Apply flows work with proper notifications (SKL-MATCH-01, SKL-MATCH-02, SKL-MATCH-03).
- Feedback system updates trust scores correctly (SKL-FEEDBACK-01, SKL-FEEDBACK-02, SKL-FEEDBACK-03).
- Status transitions are enforced — can't skip from Open to Completed (SKL-POST-03, SKL-MATCH-05).
- Posts are filterable by topic, format, compensation type, and university (SKL-BOARD-01, SKL-BOARD-02).
